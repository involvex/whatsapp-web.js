    ...
    async getSubscribers(limit) {
        return await this.client.pupPage.evaluate(
            async (channelId, limit) => {
                ...
                !limit &&
                    (limit =
                        window.Store.ChannelUtils.getMaxSubscriberNumber());
                const response =
                    await window.Store.ChannelSubscribers.mexFetchNewsletterSubscribers(
                        channelId,
                        limit,
                    );
                const contacts =
                    window.Store.ChannelSubscribers.getSubscribersInContacts(
    ...
    async _setChannelMetadata(value, property) {
        return await this.client.pupPage.evaluate(
            async (channelId, value, property) => {
                ...
                try {
                    await window.Store.ChannelUtils.editNewsletterMetadataAction(
                        channel,
                        property,
                        value,
                    );
    ...
    async _muteUnmuteChannel(action) {
        return await this.client.pupPage.evaluate(
            async (channelId, action) => {
                try {
                    action === 'MUTE'
                        ? await window.Store.ChannelUtils.muteNewsletter([
                            channelId,
                        ])
                        : await window.Store.ChannelUtils.unmuteNewsletter([
                            channelId,
                        ]);
                    return true;
                } catch (err) {
