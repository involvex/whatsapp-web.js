            ...
            async (msgId, everyone, clearMedia) => {
                ...
                if (everyone && canRevoke) {
                    return window.compareWwebVersions(
                        ...
                        ? window.Store.Cmd.sendRevokeMsgs(
                            chat,
                            { list: [msg], type: 'message' },
                            { clearMedia: clearMedia },
                        )
                        : window.Store.Cmd.sendRevokeMsgs(chat, [msg], {
                            clearMedia: true,
                ...
                return window.compareWwebVersions(
                    ...
                    ? window.Store.Cmd.sendDeleteMsgs(
                        chat,
                        { list: [msg], type: 'message' },
                        clearMedia,
                    )
    ...
    async editScheduledEvent(editedEventObject) {
        ...
        const edittedEventMsg = await this.client.pupPage.evaluate(
            async (msgId, editedEventObject) => {
                ...
                await window.Store.ScheduledEventMsgUtils.sendEventEditMessage(
                    eventOptions,
                    msg,
                );
