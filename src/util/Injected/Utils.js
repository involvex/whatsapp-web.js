...
exports.LoadUtils = () => {
    ...
    window.WWebJS.forwardMessage = async (chatId, msgId) => {
        ...
        return window.Store.ForwardUtils.forwardMessages(
            chat,
            [msg],
            true,
            true,
    ...
    window.WWebJS.sendMessage = async (chat, content, options = {}) => {
        ...
        if (options.linkPreview) {
            ...
            if (link) {
                let preview =
                    await window.Store.LinkPreview.getLinkPreview(link);
        ...
        if (isChannel) {
            ...
            const msgDataFromMsgModel =
                window.Store.SendChannelMessage.msgDataFromMsgModel(msg);
            const isMedia = Object.keys(mediaOptions).length > 0;
            await window.Store.SendChannelMessage.addNewsletterMsgsRecords([
                msgDataFromMsgModel,
            ]);
            chat.msgs.add(msg);
            chat.t = msg.t;

            const sendChannelMsgResponse =
                await window.Store.SendChannelMessage.sendNewsletterMessageJob({
                    msg: msg,
                    type:
                        message.type === 'chat'
                            ? 'text'
            ...
            await window.Store.SendChannelMessage.updateNewsletterMsgRecord(
                msg,
            );
            return msg;
        }

        const [msgPromise, sendMsgResultPromise] =
            window.Store.SendMessage.addAndSendMsgToChat(chat, message);
        await msgPromise;
    ...
    window.WWebJS.processMediaData = async (
        ...
        const file = window.WWebJS.mediaInfoToFile(mediaInfo);
        const opaqueData = await window.Store.OpaqueData.createFromData(
            file,
            file.type,
        );
        const mediaParams = {
        ...
        const dataToUpload = {
            ...
            ...(sendToChannel
                ? {
                    calculateToken:
                          window.Store.SendChannelMessage.getRandomFilehash,
                }
                : {}),
        };
        ...
        const uploadedMedia = !sendToChannel
            ...
            : await window.Store.MediaUpload.uploadUnencryptedMedia(
                dataToUpload,
            );
    ...
    window.WWebJS.getChat = async (chatId, { getAsModel = true } = {}) => {
        ...
        if (isChannel) {
            try {
                ...
                if (!chat) {
                    await window.Store.ChannelUtils.loadNewsletterPreviewChat(
                        chatId,
                    );
                    chat =
                        await window.Store.NewsletterCollection.find(chatWid);
                }
            } catch (err) {
                chat = null;
            }
            ...
            chat =
                ...
                (
                    await window.Store.FindOrCreateChat.findOrCreateLatestChat(
                        chatWid,
                    )
                )?.chat;
        }
    ...
    window.WWebJS.getContact = async (contactId) => {
        ...
        const bizProfile =
            await window.Store.BusinessProfile.fetchBizProfile(wid);
    ...
    window.WWebJS.deletePicture = async (chatid) => {
        ...
        try {
            ...
            const res =
                await window.Store.GroupUtils.requestDeletePicture(chatWid);
            return res ? res.status === 200 : false;
        } catch (err) {
            if (err.name === 'ServerStatusCodeError') return false;
            throw err;
    ...
    window.WWebJS.getAddParticipantsRpcResult = async (
        ...
        const participantArgs = [
            {
                participantJid:
                    window.Store.WidToJid.widToUserJid(participantWid),
            },
        ];
    ...
    window.WWebJS.subscribeToUnsubscribeFromChannel = async (
        ...
        try {
            if (action === 'Subscribe') {
                await window.Store.ChannelUtils.subscribeToNewsletterAction(
                    channel,
                    options,
                );
            } else if (action === 'Unsubscribe') {
                await window.Store.ChannelUtils.unsubscribeFromNewsletterAction(
                    channel,
                    options,
                );
            } else return false;
    ...
    window.WWebJS.pinUnpinMsgAction = async (msgId, action, duration) => {
        ...
        const response = await window.Store.PinnedMsgUtils.sendPinInChatMsg(
            message,
            action,
            duration,
        );
