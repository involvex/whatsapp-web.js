...
exports.ExposeLegacyStore = () => {
    ...
    window.Store.Call = window.mR.findModule(
        (module) => module.default && module.default.Call,
    )[0].default.Call;
    ...
    window.Store.GroupQueryAndUpdate = window.mR.findModule(
        'queryAndUpdateGroupMetadataById',
    )[0].queryAndUpdateGroupMetadataById;
    window.Store.Label =
    ...
    window.Store.MsgActionChecks =
        window.mR.findModule('canSenderRevokeMsg')[0];
    ...
    window.Store.getMsgInfo =
        (window.mR.findModule('sendQueryMsgInfo')[0] || {}).sendQueryMsgInfo ||
        window.mR.findModule('queryMsgInfo')[0].queryMsgInfo;
    window.Store.pinUnpinMsg =
        window.mR.findModule('sendPinInChatMsg')[0].sendPinInChatMsg;

    /* eslint-disable no-undef, no-cond-assign */
    window.Store.QueryExist = (m = window.mR.findModule('queryExists')[0])
        ? m.queryExists
        : window.mR.findModule('queryExist')[0].queryWidExists;
    window.Store.ReplyUtils =
        (m = window.mR.findModule('canReplyMsg')).length > 0 && m[0];
    /* eslint-enable no-undef, no-cond-assign */

    window.Store.Settings = {
        ...window.mR.findModule('ChatlistPanelState')[0],
        setPushname: window.mR.findModule(
            (m) => m.setPushname && !m.ChatlistPanelState,
        )[0].setPushname,
    };
    ...
    window.injectToFunction = (target, callback) => {
        ...
        const modifiedFunction = (...args) =>
            callback(originalFunction, ...args);
