...
class Client extends EventEmitter {
                ...
                pollVoteModule.bulkUpsert = (async (...args) => {
                    const votes = await Promise.all(
                        args[0].map(async (vote) => {
                            ...
                            let parentMessage = window.Store.Msg.get(
    ...
    async getPinnedMessages(chatId) {
        const pinnedMsgs = await this.pupPage.evaluate(async (chatId) => {
            ...
            const msgs = await window.Store.PinnedMsgUtils.getTable().equals(
                ['chatId'],
                chatWid.toString(),
            );

            const pinnedMsgs = msgs.map((msg) =>
                window.Store.Msg.get(msg.parentMsgKey),
            );
    ...
    async acceptChannelAdminInvite(channelId) {
        return await this.pupPage.evaluate(async (channelId) => {
            try {
                await window.Store.ChannelUtils.acceptNewsletterAdminInvite(
                    channelId,
                );
                return true;
            } catch (err) {
        ...
        return await this.pupPage.evaluate(
            async (channelId, userId) => {
                try {
                    ...
                    await window.Store.ChannelUtils.revokeNewsletterAdminInvite(
                        channelId,
                        userWid,
                    );
                    return true;
    ...
    async demoteChannelAdmin(channelId, userId) {
        return await this.pupPage.evaluate(
            async (channelId, userId) => {
                try {
                    const userWid = window.Store.WidFactory.createWid(userId);
                    await window.Store.ChannelUtils.demoteNewsletterAdmin(
                        channelId,
                        userWid,
                    );
                    return true;
            ...
            async (title, participants, options) => {
                ...
                for (const participant of createGroupResult.participants) {
                    ...
                    participant.wid.server == 'lid' &&
                        (participant.wid = window.Store.LidUtils.getPhoneNumber(
                            participant.wid,
                        ));
                    ...
                    if (autoSendInviteV4 && statusCode === 403) {
                        ...
                        const addParticipantResult =
                            await window.Store.GroupInviteV4.sendGroupInviteMessage(
                                window.Store.Chat.get(participant.wid) ||
                                    (await window.Store.Chat.find(
                                        participant.wid,
                                    )),
    ...
    async createChannel(title, options = {}) {
        return await this.pupPage.evaluate(
            async (title, options) => {
                ...
                try {
                    response =
                        await window.Store.ChannelUtils.createNewsletterQuery({
                            name: title,
                ...
                return {
                    title: title,
                    nid: window.Store.JidToWid.newsletterJidToWid(
                        response.idJid,
                    ),
    ...
    async transferChannelOwnership(channelId, newOwnerId, options = {}) {
        return await this.pupPage.evaluate(
            async (channelId, newOwnerId, options) => {
                ...
                const newOwner =
                    window.Store.Contact.get(newOwnerId) ||
                    (await window.Store.Contact.find(newOwnerId));
                if (!channel.newsletterMetadata) {
                    await window.Store.NewsletterMetadataCollection.update(
                        channel.id,
                    );
                }

                try {
                    await window.Store.ChannelUtils.changeNewsletterOwnerAction(
                        channel,
                        newOwner,
                    );
                    ...
                    if (options.shouldDismissSelfAsAdmin) {
                        const meContact =
                            window.Store.ContactCollection.getMeContact();
                        meContact &&
                            (await window.Store.ChannelUtils.demoteNewsletterAdminAction(
                                channel,
                                meContact,
                            ));
                    }
        ...
        return await this.pupPage.evaluate(
            async ({
                ...
                countryCodes =
                    ...
                    countryCodes[0] === currentRegion
                        ...
                        : countryCodes.filter((code) =>
                            Object.keys(
                                window.Store.ChannelUtils.countryCodesIso,
                            ).includes(code),
                        );
                ...
                const originalFunction =
                    window.Store.ChannelUtils.getNewsletterDirectoryPageSize;
                limit !== 50 &&
                    (window.Store.ChannelUtils.getNewsletterDirectoryPageSize =
                        () => limit);

                const channels = (
                    await window.Store.ChannelUtils.fetchNewsletterDirectories(
                        searchOptions,
                    )
                ).newsletters;
    ...
    async setBackgroundSync(flag) {
        return await this.pupPage.evaluate(async (flag) => {
            const backSync =
                window.Store.Settings.getGlobalOfflineNotifications();
            if (backSync === flag) {
                return flag;
            }
    ...
    async createCallLink(startTime, callType) {
        ...
        return await this.pupPage.evaluate(
            async (startTimeTs, callType) => {
                const response =
                    await window.Store.ScheduledEventMsgUtils.createEventCallLink(
                        startTimeTs,
                        callType,
                    );
                return response ?? '';
    ...
    async sendResponseToScheduledEvent(response, eventMessageId) {
        ...
        return await this.pupPage.evaluate(
            async (response, msgId) => {
                ...
                await window.Store.ScheduledEventMsgUtils.sendEventResponseMsg(
                    response,
                    eventMsg,
                );
                return true;
    ...
    async deleteAddressbookContact(phoneNumber) {
        return await this.pupPage.evaluate(async (phoneNumber) => {
            return await window.Store.AddressbookContactUtils.deleteContactAction(
                phoneNumber,
            );
        }, phoneNumber);
    }
    ...
    async addOrEditCustomerNote(userId, note) {
        return await this.pupPage.evaluate(
            async (userId, note) => {
                if (!window.Store.BusinessGatingUtils.smbNotesV1Enabled())
                    return;

                return window.Store.CustomerNoteUtils.noteAddAction(
                    'unstructured',
                    window.Store.WidToJid.widToUserJid(
                        window.Store.WidFactory.createWid(userId),
                    ),
                    note,
                );
    ...
    async getCustomerNote(userId) {
        return await this.pupPage.evaluate(async (userId) => {
            ...
            const note =
                await window.Store.CustomerNoteUtils.retrieveOnlyNoteForChatJid(
                    window.Store.WidToJid.widToUserJid(
                        window.Store.WidFactory.createWid(userId),
                    ),
                );
            ...
            serialized.chatId = window.Store.JidToWid.userJidToUserWid(
                serialized.chatJid,
            )._serialized;
